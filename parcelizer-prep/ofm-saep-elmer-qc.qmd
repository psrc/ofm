...
This document provides quality checks on the latest SAEP Vintage before it is finalized in Elmer. Unstaged data refers to `r dir` . The last vintage in Elmer at the time of this report refers to the 2024 Vintage.
...
```{r data apr1, eval=TRUE}
source(here::here("parcelizer-prep/03-compare-to-apr1.R"))

dsa <- df_saep_apr |> 
  select(YEAR, COUNTYNAME, POP_diff, HU_diff) |> 
  pivot_wider(id_cols = "YEAR", names_from = COUNTYNAME, values_from = c("POP_diff", "HU_diff"))
```

```{r plot apr1, eval=TRUE}
dsa |> 
  gt(rowname_col = "YEAR") |>
  fmt_number(columns = everything(),
             decimals = 0,
  ...
  cols_label(
    YEAR = "Year",
    `POP_diff_King` = "Pop \U0394",
    `HU_diff_King`  = "HU \U0394",
    `POP_diff_Kitsap` = "Pop \U0394",
    `HU_diff_Kitsap`  = "HU \U0394",
    `POP_diff_Pierce` = "Pop \U0394",
    `HU_diff_Pierce`  = "HU \U0394",
    `POP_diff_Snohomish` = "Pop \U0394",
    `HU_diff_Snohomish`  = "HU \U0394"
  )
```
